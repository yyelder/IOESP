...
/**
 * ipc - 确保在非Electron环境中也有默认值
 */
const ipc = Renderer.ipcRenderer || {
  // 提供默认的空实现，避免在非Electron环境中出错
  invoke: () => Promise.resolve('Not in Electron environment'),
  sendSync: () => 'Not in Electron environment',
  on: () => {},
  once: () => {},
  removeListener: () => {},
  removeAllListeners: () => {},
 ...
 * 是否为EE环境
 */
const isEE = Renderer.ipcRenderer ? true : false;

// 注意：contextBridge 只能在预加载脚本中使用，不能在渲染进程中直接使用
// 如果需要使用 contextBridge，应该在 electron/preload/index.js 中定义
